...
class RegisterScreen extends StatelessWidget {
  ...
  Future<void> _pickImage() async {
    ...
    if (image != null) {
      ...
      Get.snackbar('Muvaffaqiyatli'.tr, 'Surat tanlandi'.tr, backgroundColor: AppColors.lightBlue, colorText: AppColors.white);
    }
  }
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
                      ...
                      child: Stack(
                        ...
                        children: [
                          registerController.selectedImage.value != null
                              ? Image.file(registerController.selectedImage.value!, fit: BoxFit.cover)
                              : Icon(LucideIcons.user, color: AppColors.lightGray, size: Responsive.scaleFont(50, context)),
                          Align(alignment: Alignment.bottomCenter, child: Container(height: Responsive.scaleHeight(35, context), color: AppColors.darkBlue.withOpacity(0.7))),
                          Align(
                            alignment: Alignment.bottomCenter,
                            child: IconButton(icon: Icon(LucideIcons.camera, color: AppColors.lightBlue, size: Responsive.scaleFont(20, context)), onPressed: _pickImage)
                          )
                        ]
                      )
                    ))
              ...
              Obx(() => Stack(
                children: [
                  _buildDropdown(context, registerController.regions, registerController.selectedRegionId.value, (newValue) {
                  ...
                  if (registerController.regions.isEmpty && !registerController.isLoadingRegions.value)
              ...
              Obx(() => Stack(
                children: [
                  _buildDropdown(context, registerController.districts, registerController.selectedDistrictId.value, (newValue) {if (newValue != null) {registerController.selectedDistrictId.value = newValue;}}, 'Tuman', LucideIcons.mapPin),
                  if (registerController.isLoadingDistricts.value)
                    Positioned(
                      right: Responsive.scaleWidth(10, context),
                      top: Responsive.scaleHeight(10, context),
                      child: SizedBox(width: Responsive.scaleWidth(20, context), height: Responsive.scaleHeight(20, context), child: CircularProgressIndicator(color: AppColors.lightBlue, strokeWidth: 2))
                    ),
                  if (registerController.districts.isEmpty && !registerController.isLoadingDistricts.value && registerController.selectedRegionId.value.isNotEmpty)
                    Positioned(child: Text('Tumanlar topilmadi'.tr, style: TextStyle(color: AppColors.red, fontSize: Responsive.scaleFont(12, context))))
                ]
              )),
              SizedBox(height: Responsive.scaleHeight(24, context)),
              Obx(() => ElevatedButton(
                onPressed: registerController.isRegistering.value ? null : () {
                  if (_formKey.currentState!.validate()) {
                    if (registerController.selectedGender.value.isEmpty) {
                      Get.snackbar('Xatolik'.tr, 'Jins tanlanishi shart'.tr, backgroundColor: AppColors.red.withOpacity(0.8), colorText: AppColors.white);
                      return;
                    }
                    if (registerController.selectedRegionId.value.isEmpty || registerController.isLoadingRegions.value) {
                      Get.snackbar('Xatolik'.tr, 'Viloyat tanlanishi shart'.tr, backgroundColor: AppColors.red.withOpacity(0.8), colorText: AppColors.white);
                      return;
                    }
                    if (registerController.selectedDistrictId.value == '0' ||
                        registerController.isLoadingDistricts.value) {
                      Get.snackbar('Xatolik'.tr, 'Tuman tanlanishi shart'.tr, backgroundColor: AppColors.red.withOpacity(0.8), colorText: AppColors.white);
                      return;
                    }
                    ...
                    ShowToast.show('Xatolik'.tr, 'Iltimos, barcha majburiy maydonlarni to‘ldiring'.tr, 3, 1, duration: 2);
                  }
                },
                ...
                child: registerController.isRegistering.value
                    ? SizedBox(width: Responsive.scaleWidth(20, context), height: Responsive.scaleHeight(20, context), child: CircularProgressIndicator(color: AppColors.white, strokeWidth: 2))
    ...
    return TextFormField(
      ...
      onTap: () async {
        DateTime? pickedDate = await showDatePicker(
          ...
          builder: (context, child) => Theme(data: ThemeData.dark().copyWith(colorScheme: ColorScheme.dark(primary: AppColors.lightBlue, onPrimary: AppColors.white, surface: AppColors.darkBlue)), child: child!)
        );
        if (pickedDate != null) {
  ...
  Widget _buildDropdown(BuildContext context, List<Map<String, dynamic>> items, String value, Function(String?) onChanged, String label, IconData icon) {
    return DropdownButtonFormField2<String>(
      ...
      isExpanded: true,
      validator: (val) =>
      val == null || val.isEmpty || val == '0' ? '$label ${'tanlanishi shart'.tr}' : null,
      decoration: InputDecoration(
        prefixIcon: Icon(icon, color: AppColors.lightGray, size: Responsive.scaleFont(18, context)),
        labelText: label,
        labelStyle:
      ...
      items: items.map<DropdownMenuItem<String>>((item) {
        return DropdownMenuItem<String>(
          ...
          child: Text(item['name'] ?? 'Noma’lum'.tr, style: TextStyle(fontSize: Responsive.scaleFont(14, context)), overflow: TextOverflow.ellipsis)
        );
      }).toList(),
