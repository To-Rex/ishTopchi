...
class _AdPostingScreenState extends State<AdPostingScreen> with TickerProviderStateMixin {
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildTextField(context, controller.titleController, 'E’lon sarlavhasi *', 'Masalan: Yangi ish imkoniyati', maxLines: 1, keyboardType: TextInputType.text, validator: (value) => value == null || value.trim().isEmpty ? 'Sarlavha kiritilishi shart' : null, fieldKey: _titleFieldKey),
              SizedBox(height: AppDimensions.paddingMedium),
              _buildTextField(context, controller.contentController, 'Tavsif *', 'Tavsif yozing...', maxLines: 4, keyboardType: TextInputType.multiline, validator: (value) => value == null || value.trim().isEmpty ? 'Tavsif kiritilishi shart' : null, fieldKey: _contentFieldKey),
              SizedBox(height: AppDimensions.paddingMedium),
              Row(
                children: [
                  Expanded(child: _buildTextField(context, controller.salaryFromController, 'Oylik (dan)', '50000', maxLines: 1, keyboardType: TextInputType.number, fieldKey: _salaryFromFieldKey)),
                  SizedBox(width: AppDimensions.paddingMedium),
                  Expanded(child: _buildTextField(context, controller.salaryToController, 'Oylik (gacha)', '70000', maxLines: 1, keyboardType: TextInputType.number, fieldKey: _salaryToFieldKey))
                ]
              ),
              SizedBox(height: AppDimensions.paddingMedium),
              _buildDropdownField<int>(context, 'Kategoriya *', controller.categories, controller.selectedCategory,
                LucideIcons.briefcase, (value) => controller.selectedCategory.value = value ?? 1,
                validator: (value) => value == null || value == 0 ? 'Kategoriya tanlanishi shart' : null,
                isInt: true,
                fieldKey: _categoryFieldKey,
              ),
              SizedBox(height: AppDimensions.paddingMedium),
              _buildDropdownField<String>(context, 'Viloyat *', controller.regions, controller.selectedRegionId,
              ...
              SizedBox(height: AppDimensions.paddingMedium),
              _buildDropdownJobType<String>(context, 'Bandlik turi', employmentTypes, controller.selectedEmploymentType, LucideIcons.clock, (value) => controller.selectedEmploymentType.value = value!, fieldKey: _employmentTypeFieldKey),
              SizedBox(height: AppDimensions.paddingMedium),
              _buildMapPicker(context),
              SizedBox(height: AppDimensions.paddingMedium),
              _buildTextField(context, controller.phoneNumberController, 'Telefon raqami *', '+998 99 534 03 13', maxLines: 1, keyboardType: TextInputType.phone, inputFormatters: [phoneFormatter], validator: _validatePhone, fieldKey: _phoneFieldKey),
              SizedBox(height: AppDimensions.paddingMedium),
              _buildTextField(context, controller.emailController, 'Email', 'torex.amaki@gmail.com', maxLines: 1, keyboardType: TextInputType.emailAddress, fieldKey: _emailFieldKey),
  ...
  Widget _buildDropdownJobType<T>(BuildContext context, String label, List<dynamic> items, Rx<T?> selectedValue, IconData icon, Function(T?) onChanged, {String? Function(T?)? validator, bool isInt = false, GlobalKey<FormFieldState>? fieldKey}) =>
      DropdownButtonFormField2<T>(
          ...
          items: items.map((item) => DropdownMenuItem<T>(value: isInt ? item['id'] as T : item['value'] as T, child: Text(item['title'] ?? item['name'] ?? 'Noma’lum', style: TextStyle(fontSize: Responsive.scaleFont(14, context)), overflow: TextOverflow.ellipsis))).toList(),
          onChanged: onChanged,
    ...
    child: AnimatedContainer(
      ...
      child: controller.selectedImage.value == null ? Column(
        ...
        children: [
          ...
          Text('Rasm yuklash', style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontSize: Responsive.scaleFont(16, context), color: AppColors.lightBlue))
        ]
      ) : ClipRRect(
        borderRadius: BorderRadius.circular(AppDimensions.cardRadius),
        child: Image.file(
